    ...
    drawContours(land, {levels = 18, seed = 1} = {}) {
        ...
        for (let py = 0; py < this.h; py++) {
            for (let px = 0; px < this.w; px++) {
                const [wx, wy] = this.toWorld(px + 0.5, py + 0.5);
                const v = land.f(wx, wy);
                const tnorm = Math.max(0, Math.min(1, (Math.log(v - fmin + 1)) / (Math.log(fmax - fmin + 1) + 1e-9)));
    ...
    _marchingSquares(grid, level, nx, ny) {
        ...
        for (let j = 0; j < ny; j++) {
            for (let i = 0; i < nx; i++) {
                ...
                const P = [[gx(i), gy(j)], [gx(i + 1), gy(j)],
                    [gx(i + 1), gy(j + 1)], [gx(i), gy(j + 1)]];
                const V = [v0, v1, v2, v3];
                for (let e = 0; e < 4; e++) {
                    const a = e, b = (e + 1) % 4;
