...
export function makeAdam({mode = 'adam', beta1 = 0.9, beta2 = 0.999, eps = 1e-8, lr = 0.05} = {}) {
    let m = [0, 0], v = [0, 0], t = 0, vel = [0, 0];
    return {
        step(land, x, seed = 1) {
            t += 1;
            const g = land.noisy ? land.grad(x[0], x[1], seed) : land.grad(x[0], x[1]);
            ...
            v = [beta2 * v[0] + (1 - beta2) * g[0] * g[0],
                beta2 * v[1] + (1 - beta2) * g[1] * g[1]];
            const mh = [m[0] / (1 - beta1 ** t), m[1] / (1 - beta1 ** t)];
            const vh = [v[0] / (1 - beta2 ** t), v[1] / (1 - beta2 ** t)];
            const step = [lr * mh[0] / (Math.sqrt(vh[0]) + eps),
                lr * mh[1] / (Math.sqrt(vh[1]) + eps)];
            return {next: sub(x, step), vel: m.slice(), scale: [Math.sqrt(vh[0]) + eps, Math.sqrt(vh[1]) + eps]};
        },
        reset() {
            m = [0, 0];
            v = [0, 0];
