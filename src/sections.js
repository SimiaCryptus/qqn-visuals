...
import {pathPolyline, pathPoint, pathTangent, armijoSearch, originDirectional} from './qqn-path.js';
...
const C = {
    gradient: '#0072B2', oracle: '#E69F00', path: '#009E73',
    accepted: '#CC79A7', bracket: '#999', iterate: '#D55E00',
};
...
function runQQN(land, start, n = 40) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        const gradDir = neg(g), oracleDir = dir;
...
export function buildSections(global) {
    ...
    S.push({
        id: 'landscape', title: 'What is a loss landscape?',
    ...
    S.push({
        id: 'optimizer', title: 'What is an optimizer?',
        ...
        build(ctx) {
            ...
            let x = global.start.slice(), pts = [x.slice()];
    ...
    S.push({
        ...
        build(ctx) {
            ...
            ${legend([[C.iterate, 'trajectory'], [C.gradient, 'velocity']])}`;
            ctx.container.querySelector('.viz').appendChild(pc);
            const sc = new Scene2D(ctx.canvas, LANDSCAPES.noisy.domain);
            let mode = 'gd', b1 = 0.9, b2 = 0.999;
            const draw = () => {
                const L = LANDSCAPES[global.landscapeKey];
                sc.setDomain(L.domain);
                sc.clear();
            ...
            pc.querySelectorAll('[data-mode]').forEach((b) => b.addEventListener('click', () => {
    ...
    // ---- Section 5: L-BFGS ----
    S.push({
        id: 'lbfgs', title: 'L-BFGS (quasi-Newton)',
        ...
        build(ctx) {
            ...
            ${legend([[C.oracle, '−H∇f'], [C.iterate, 'trajectory']])}`;
            ctx.container.querySelector('.viz').appendChild(pc);
    ...
    // ---- Section 6: Trade-off ----
    S.push({
        id: 'tradeoff', title: 'The trade-off',
        ...
        build(ctx) {
            ...
            ${legend([[C.gradient, 'GD'], [C.oracle, 'L-BFGS']])}`;
            ctx.container.querySelector('.viz').appendChild(pc);
            const sc = new Scene2D(ctx.canvas, LANDSCAPES.illcond.domain);
            let raf = null;
            const draw = (nGd, nLb) => {
    ...
    // ---- Section 7: QQN blend ----
    S.push({
        id: 'qqn', title: 'QQN: blend, don\'t choose',
        ...
        build(ctx) {
            ...
            ${legend([[C.gradient, '−∇f'], [C.oracle, '−H∇f (drag)'], [C.path, 'd(t)']])}`;
            ctx.container.querySelector('.viz').appendChild(pc);
            const land = LANDSCAPES[global.landscapeKey];
            const sc = new Scene2D(ctx.canvas, land.domain);
            let t = 0.5;
            ...
            const draw = () => {
                ...
                pc.querySelector('#ro-7').textContent =
                    `t=${t.toFixed(2)}  f(x+d(t))=${fv.toFixed(4)}`;
            };
            ctx.canvas.addEventListener('pointerdown', (e) => {
                const r = ctx.canvas.getBoundingClientRect();
                const w = sc.toWorld(e.clientX - r.left, e.clientY - r.top);
                if (norm([w[0] - (x[0] + oracleDir[0]), w[1] - (x[1] + oracleDir[1])]) < 0.3) dragging = true;
            });
    ...
    S.push({
        id: 'props', title: 'The path\'s three properties',
        ...
        build(ctx) {
            ...
            const draw = () => {
                ...
                pc.querySelector('#ro-8').textContent =
                    `⟨∇f, d'(0)⟩ = −‖∇f‖² = ${dd.toFixed(4)}`;
            };
    ...
    S.push({
        id: 'linesearch', title: 'Line search: walking t',
        ...
        build(ctx) {
            ctx.container.querySelector('.viz').innerHTML =
             ...
             <div class="panel-controls">
               ...
               ${legend([[C.path, 'φ(t)'], [C.bracket, 'Armijo line'], [C.accepted, 'accepted']])}
               <div id="sw-warn"></div>
             </div>`;
            const land = LANDSCAPES[global.landscapeKey];
            ...
            const draw = () => {
                ...
                p1.clear();
                p1.curve(phi, C.path);
                const f0 = phi(0), phi0p = dot(g, gradDir);
                p1.line(0, f0, 1, f0 + c1 * phi0p, C.bracket);
                // run search
                const res = armijoSearch(land, x, g, gradDir, dir,
                    {c1, tStart: strat === 'fixed' ? 1 : 1, maxIter: strat === 'fixed' ? 1 : 20});
            ...
            ctx.container.querySelectorAll('[data-ls]').forEach((b) => b.addEventListener('click', () => {
        ...
        build(ctx) {
            ctx.container.querySelector('.viz').innerHTML =
             ...
             <div class="panel-controls">
               ...
               ${legend([[C.path, 'spline'], [C.oracle, 'tangent stub'], [C.accepted, 'stationary pt']])}
             </div>`;
            const p1 = new Plot1D(ctx.container.querySelector('.p1d'));
    ...
    S.push({
        ...
        build(ctx) {
            ...
            const clipBox = (p) => [Math.max(-1.5, Math.min(1.5, p[0])), Math.max(-1.5, Math.min(1.5, p[1]))];
            const draw = () => {
                const L = LANDSCAPES[global.landscapeKey];
                sc.setDomain(L.domain);
                sc.clear();
    ...
    S.push({
        id: 'equiv', title: 'Equivalences (recap)',
        ...
        build(ctx) {
            ...
            const pc = document.createElement('div');
            pc.className = 'panel-controls';
            pc.innerHTML = `<div class="preset-grid">${Object.keys(presets).map((k) =>
                `<button class="btn" data-p="${k}">${k}</button>`).join('')}</div>
            <table class="eq-table"><tr><th>Method</th><th>Oracle</th><th>t regime</th></tr>
            ${Object.entries(presets).map(([k, v]) =>
                `<tr data-p="${k}"><td>${k}</td><td>${v.oracle}</td><td>${v.tmax >= 1 ? 't=1' : 't→0'}</td></tr>`).join('')}
            </table>`;
            ctx.container.querySelector('.viz').appendChild(pc);
            const sc = new Scene2D(ctx.canvas, LANDSCAPES[global.landscapeKey].domain);
            const run = (preset) => {
                ...
                for (let i = 0; i < 30; i++) {
                    ...
                    let nx = [x[0] + d[0], x[1] + d[1]];
                    if (preset.region === 'box') nx = [Math.max(-1.5, Math.min(1.5, nx[0])), Math.max(-1.5, Math.min(1.5, nx[1]))];
                    x = nx;
                    pts.push(x.slice());
                    if (norm(g) < 1e-4) break;
                }
